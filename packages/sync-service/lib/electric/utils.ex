defmodule Electric.Utils do
  ...
  aim of this obfuscation is to avoid accidentally leaking the password when inspect connection
  opts or logging them as part of a process state (which is done automatically by OTP when an
  OTP behaviour crashes).
  """
  ...
  @doc """
  Undo the obfuscation applied by `obfuscate_password`.

  This function should be called just before passing connection options to one of
  `Postgrex.*.start_link()` functions. Never store deobfuscated password in any of our process
  states.
  """
